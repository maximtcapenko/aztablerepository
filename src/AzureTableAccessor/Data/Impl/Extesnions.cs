namespace AzureTableAccessor.Data.Impl
{
    ...
    {
        internal static (object partitionKey, object rowKey) GetKeysFromEntity<TEntity>(
            this IEnumerable<IPropertyRuntimeMapper<TEntity>> mappers, TEntity entity) where TEntity : class
        {
            var configurations = mappers.OfType<IPropertyConfigurationProvider<TEntity>>()
        ...
        internal static IEnumerable<IPropertyConfiguration<TEntity, TProjection>> ToPropertyConfigurations<TEntity, TProjection>(
            this IEnumerable<IPropertyRuntimeMapper<TEntity, TProjection>> mappers)
            where TEntity : class
            where TProjection : class
             => mappers.OfType<IPropertyConfigurationProvider<TEntity, TProjection>>()
                       .Select(e => e.GetPropertyConfiguration());

        internal static IEnumerable<IPropertyConfiguration<TEntity>> ToPropertyConfigurations<TEntity>(
            this IEnumerable<IPropertyRuntimeMapper<TEntity>> mappers)
            where TEntity : class
             => mappers.OfType<IPropertyConfigurationProvider<TEntity>>()
                       .Select(e => e.GetPropertyConfiguration());
